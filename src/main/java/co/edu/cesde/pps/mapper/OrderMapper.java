...
public class OrderMapper {
    ...
    public OrderDTO toDTO(Order order) {
        ...
        if (order.getStatus() != null) {
            dto.setOrderStatusName(order.getStatus().getName());
        }

        dto.setCreatedAt(order.getCreatedAt());

        // Convertir items (defensivo para evitar LazyInitialization en contextos fuera de transacción)
        try {
            ...
            // Si la colección es LAZY y no está inicializada, evitamos romper la serialización.
            dto.setItemsCount(0);
        }
     ...
     */
    /**
     * NOTA: Este método no debe ser usado para crear Order con JPA.
     * Order requiere entidades completas (User, OrderStatus, Address).
     * Usar el constructor de Order directamente en el servicio.
     */
    @Deprecated
    public Order toEntity(OrderDTO dto) {
        // Este método ya no es funcional con JPA
        // Order debe crearse con entidades completas en OrderService
        throw new UnsupportedOperationException(
            "toEntity no soportado. Usar constructor de Order con entidades completas en OrderService"
        );
    }
