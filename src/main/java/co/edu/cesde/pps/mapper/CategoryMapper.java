...
public class CategoryMapper {
    ...
    public CategoryDTO toDTO(Category category) {
        ...
        dto.setIsRoot(category.isRootCategory());

        // Campos agregados (defensivo por colecciones LAZY)
        try {
            dto.setSubcategoriesCount(category.getSubcategories() != null ? category.getSubcategories().size() : 0);
        } catch (RuntimeException ex) {
            dto.setSubcategoriesCount(0);
        }

        try {
            dto.setProductsCount(category.getProducts() != null ? category.getProducts().size() : 0);
        } catch (RuntimeException ex) {
    ...
    public CategoryDTO toDTOWithHierarchy(Category category) {
        ...
        // Convertir subcategorías recursivamente (defensivo por LAZY)
        try {
            ...
            // Si LAZY no está inicializada, se omite jerarquía.
        }
